// Yahoo LIVE snake-draft driver — REAL ROOM ONLY (league 384341, team 6,
// "All I Do Is Win" — dad's league; owner-confirmed). Byte-for-byte the
// validated mock #8 loop (endgame hard gate, select-dropdown list switch,
// count-verified one-click-max picks) with the yahoo_actuate.mjs gate
// sequence bolted on:
//   - refuses every mock/foreign room (exact inverse of the mock driver)
//   - DRY-RUN by default: full loop, logs every would-be click, clicks NOTHING
//   - --live requires --grant-file: alias allowlist (allidoiswin only,
//     RoughRydas forbidden forever), league 384341, valid time window, and
//     draft_session_id matching 470.l.384341-2026-<start_ms>
//   - grant expiry re-checked before EVERY live click
// Artifacts are live_-prefixed so mock artifacts are never clobbered.
...
const LEAGUE = 384341, TEAMID = 6, GAME_KEY = 470;
const repo = fileURLToPath(new URL("..", import.meta.url));
mkdirSync(join(repo, "data", "yahoo"), { recursive: true });
const D = (f) => join(repo, "data", "yahoo", "live_" + f);
const log = (m) => { const l = `${new Date().toISOString()} ${m}`; console.log(l); appendFileSync(D("driver.log"), l + "\n"); };
const die = (msg) => { log(`REFUSED: ${msg}`); process.exit(3); };

// ---- gate sequence (mirrors yahoo_actuate.mjs / ESPN conventions) ----------
const argv = process.argv.slice(2);
const LIVE = argv.includes("--live");
const grantIdx = argv.indexOf("--grant-file");
let DRY_RUN = true;
if (LIVE) {
  if (grantIdx === -1 || !argv[grantIdx + 1]) die("--live requires --grant-file");
  let grant;
  try { grant = JSON.parse(readFileSync(argv[grantIdx + 1], "utf8")); } catch { die("grant file unreadable or not JSON"); }
  const alias = String(grant.alias || "").trim().toLowerCase();
  if (alias === "roughrydas") die("RoughRydas is forbidden — never act on it");
  if (alias !== "allidoiswin") die(`grant alias ${JSON.stringify(grant.alias)} is not allowlisted`);
  if (grant.league_id !== LEAGUE) die(`grant league ${grant.league_id} != ${LEAGUE}`);
  if (!/^470\.l\.384341-2026-\d{13}$/.test(String(grant.draft_session_id || "")))
    die("grant draft_session_id must match 470.l.384341-2026-<start_ms>");
  const now = Date.now();
  if (!(grant.issued_at_ms <= now && now < grant.expires_at_ms)) die("grant is not currently valid");
  globalThis.__grant = grant;
  DRY_RUN = false;
}
const grantStillValid = () => { const g = globalThis.__grant; return g && Date.now() < g.expires_at_ms; };
log(DRY_RUN ? "MODE: DRY-RUN (no clicks will be performed)" : "MODE: LIVE (grant validated; expiry re-checked per click)");
...
log(`yahoo LIVE driver armed: slot ${SLOT || "?"}/${TEAMS}, league ${LEAGUE} team ${TEAMID}`);

// REAL room ONLY, TWO forms (owner safety rule): /f1/384341/draft and
// draftclient/f1/384341/<slot>. Everything else — mock ids, mock_waiting,
// mock_lobby, other leagues — is refused. Exact inverse of the mock driver.
  ...
  try {
    return JSON.parse(execFileSync(join(repo, ".venv/bin/python"),
      [join(repo, "scripts/ctx_choose.py"), "--history", D("hist.txt"), "--visible", D("visible.json"),
       ...
       "--roster-file", D("roster.txt"), "--slot", String(slot), "--teams", String(TEAMS), "--league", String(LEAGUE), "--teamid", String(TEAMID)],
...
while (ourPicks.length < ROUNDS) {
  ...
  // ---- gate: dry-run never clicks; live re-checks grant expiry per click --
  if (DRY_RUN) { log(`DRY-RUN: would click ${choice.playerName} (${choice.pos}) — no click performed`); clickedThisTurn = true; continue; }
  if (!grantStillValid()) { log("HALT: grant expired mid-draft — NO click; manual takeover"); clickedThisTurn = true; continue; }
  try {
